...
void GridWidget::setAroundShipFields(const QVector<QPair<int, int>> &coordinates) {
    ...
    for(const auto& i : coordinates) {
        buttons_sea_fieald[i.first + i.second * 10]->setButtonState("UNKNOWN_FIELD");
    }
}

Button** GridWidget::getField() { return  buttons_sea_fieald; }
size_t GridWidget::getFieldCount() { return field_count; }

void GridWidget::setMissOnField(const QPair<int, int> &pos) {
    qDebug() << "SetMissOnField";
    buttons_sea_fieald[pos.first + pos.second * 10]->setButtonState("MISS_FIELD");
}

void GridWidget::setHitOnField(const QPair<int, int> &pos) {
    buttons_sea_fieald[pos.first + pos.second * 10]->setButtonState("DESTROYEDED_FIELD");
}
...
void GridWidget::setShipPositionInGrid(const QPair<int, int> &pos, bool orientation, int length)
{
    ...
    if (orientation) {
        for(int i = 0; i < length; ++i) {
             buttons_sea_fieald[pos.first + 10 * pos.second + i]->setButtonState("SHIP_FIELD");
        }
    }
    else {
        for(int i = 0; i < length; ++i) {
             buttons_sea_fieald[pos.first + 10 * pos.second + i * 10]->setButtonState("SHIP_FIELD");
        }
    }
}
...
//             buttons_sea_fieald[pos[0].first + 10 * pos[i].second + i]->setIcon(*field);
            buttons_sea_fieald[pos[0].first + 10 * pos[i].second + i]->setButtonState("EMPTY_FIELD");
        }
    }
    else {
        for(int i = 0; i < pos.size(); ++i) {
             //buttons_sea_fieald[pos[i].first + 10 * pos[0].second + i * 10]->setIcon(*field);
            buttons_sea_fieald[pos[i].first + 10 * pos[0].second + i * 10]->setButtonState("EMPTY_FIELD");
        }
    }
}
...
void GridWidget::setFields(const QSize & size)
{
    for (size_t i = 0; i < field_count; ++i)
    {
        buttons_sea_fieald[i] = new Button(this);
        buttons_sea_fieald[i]->setFixedSize(size);
